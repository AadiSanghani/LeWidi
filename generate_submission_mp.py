...
def build_input(example: dict, tokenizer: AutoTokenizer) -> str:
    ...
    reply = example["text"].get("reply", "")
    if post:
        return f"{post} {tokenizer.sep_token} {reply}"
    return reply
    ...
    with open(args.output_tsv, "w", encoding="utf-8") as out_f:
        for ex_id, ex in tqdm(data.items(), desc="Predicting"):
            ...
            if args.task == "A":
                ...
                # MP is binary classification, so we output [P(not_irony), P(irony)]
                out_probs = probs.tolist()
                # Ensure we output exactly 2 probabilities for binary classification
                if len(out_probs) < 2:
                    pad = [0.0] * (2 - len(out_probs))
                    out_probs = pad + out_probs
                elif len(out_probs) > 2:
                ...
                # Convert prob distribution to single rating (argmax for binary)
