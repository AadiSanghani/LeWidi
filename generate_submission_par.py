    ...
    with open(args.output_tsv, "w", encoding="utf-8") as out_f:
        for ex_id, ex in tqdm(dataset.data.items(), desc="Predicting"):
            ...
            if args.task == "A":
                ...
                with torch.no_grad():
                    logits = model(texts=[full_text], **demographic_inputs)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate submission TSV for Paraphrase dataset using ParDemogModel.")
